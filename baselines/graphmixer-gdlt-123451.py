...
# === import your GDELT dataset class (adjust path/module name if needed) ===
# from your_dataset_module import GDELTDataset
# from GDELTDataset import GDELTDataset  # if the class is in GDELTDataset.py in the same folder
...
class GraphMixerEncoder(torch.nn.Module):
    """
    GraphMixer-style encoder adapted to your TGN pipeline.
    - Edge token = [time_enc(Δt) || msg]
    - Token FFN (pre-norm, residual): hidden -> 0.5*hidden -> hidden
    - Mean-pool tokens to destination node
    - Channel FFN (pre-norm, residual): hidden -> 4*hidden -> hidden
    - Output projection + residual from memory embedding
    """
    def __init__(self, in_channels, out_channels, msg_dim, time_enc,
                 hidden=100, drop=0.1):
        super().__init__()
        self.time_enc = time_enc
        token_in = msg_dim + time_enc.out_channels  # [edge_feat || time_enc]

        # Encode per-edge tokens
        self.feat_linear = Linear(token_in, hidden)

        # Token FFN: hidden -> 0.5*hidden -> hidden (pre-norm + residual)
        self.token_ln   = LayerNorm(hidden)
        self.token_ffn0 = Linear(hidden, int(0.5 * hidden))
        self.token_ffn1 = Linear(int(0.5 * hidden), hidden)

        # Channel FFN on pooled node reps: hidden -> 4*hidden -> hidden
        self.channel_ln   = LayerNorm(hidden)
        self.channel_ffn0 = Linear(hidden, 4 * hidden)
        self.channel_ffn1 = Linear(4 * hidden, hidden)
    ...
    def forward(self, x, last_update, edge_index, t, msg):
        """
        x:          [N, in_channels] (memory embeddings of subgraph nodes)
        last_update:[N]              (timestamps for nodes in subgraph)
        edge_index: [2, E]           (u -> v)
        t:          [E]              (edge timestamps)
        msg:        [E, msg_dim]     (edge features, e.g., relation id as float or embedding)
        returns:    [N, out_channels]
        """
        # Build per-edge tokens
        rel_t = (last_update[edge_index[0]] - t).to(x.dtype)  # [E]
        te    = self.time_enc(rel_t)                          # [E, time_dim]
        tok   = torch.cat([te, msg], dim=-1)                  # [E, token_in]

        h = self.feat_linear(tok)                             # [E, hidden]

        # Token mixing (pre-norm + residual)
        h = self.token_ln(h)
        h_tok = self.token_ffn1(self.act(self.token_ffn0(h)))
        h = h + self.drop(h_tok)
        ...
        # Channel mixing (pre-norm + residual)
        hc = self.channel_ln(h_node)
        hc = self.channel_ffn1(self.act(self.channel_ffn0(hc)))
        h_node = h_node + self.drop(hc)

        # Output + residual from memory
        out = self.proj_out(h_node)
        out = self.out_ln(out) + self.res_proj(x)
        return out
...
def plot_tsne_node_embeddings(history, tracked_nodes, seed):
    ...
    plt.tight_layout()
    plot_filename = f"gdelt_graphmixer_seed_{seed}.png"
...
def train_epoch(loader, memory, gnn, link_pred, optimizer, neighbor_loader, assoc, data, criterion):
    ...
    for batch in loader:
        ...
        # map e_id from CPU to device and gather edge attrs
        z_tgn = gnn(
...
def evaluate(loader, memory, gnn, link_pred, neighbor_loader, assoc, data, criterion):
    ...
    # stitch subgraph embeddings back to full node space
    full_z_tgn = torch.zeros(data.num_nodes, all_z_tgn[-1].size(1))
...
def run_single_experiment(seed, data, train_data, val_loader, test_loader, device, loader_kwargs):
    ...
    for epoch in range(1, 51):
        ...
        # track best EMA(AP) -> evaluate on test
        if ema_val_ap > best_ema_val_ap:
            ...
            torch.save(
                ...
                osp.join('saved_models', f'gdelt_graphmixer_seed_{seed}.pth')
            )
        else:
...
if __name__ == '__main__':
    ...
    try:
        log_filename = f"gdelt_graphmixer_seed_{time.strftime('%Y%m%d-%H%M%S')}.txt"
        tee = Tee(log_filename)
        print(f"Using device: {device}")

        # --- Load GDELT TemporalData ---
        path = osp.join('.', 'data')  # adjust if you store elsewhere
        dataset = GDELTDataset(path, bin_15min=True)
        data = dataset[0]

        # Ensure num_nodes is set for memory and neighbor loader:
        ...
        # Dataloaders for val/test (train loader is created inside the run)
        loader_kwargs = dict(
        ...
        # Track a few nodes for t-SNE viz
