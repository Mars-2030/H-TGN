...
class BitcoinOTCDataset(InMemoryDataset):
    r"""The temporal knowledge graph dataset from the Bitcoin-OTC trust network,
    which combines all interactions into a single dataset.
    ...
    **STATS:**
    ...
    def download(self) -> None:
        # Download the gz file (pandas can read it directly)
    ...
    def process(self) -> None:
        ...
        for file_name in self.raw_file_names:
            ...
            # Columns: SOURCE, TARGET, RATING, TIME (Unix seconds)
            df = pd.read_csv(
                file_path,
                compression='gzip',
                header=None,
                names=['src', 'dst', 'relation', 'timestamp'],
                comment='#',            # skip SNAP comment/header lines
                sep=',',
                engine='c',
                low_memory=False,
            )

            # Ensure numeric types; drop malformed rows if any
        ...
        # Store the rating as relation/message feature (E,1) float — same shape as your ICEWS code:
...
def plot_tsne_node_embeddings(history, tracked_nodes, seed):
    ...
    plt.tight_layout()
    plot_filename = f"bitcoin_tgn_seed_{seed}.png"
...
def run_single_experiment(seed, data, train_data, val_loader, test_loader, device, loader_kwargs):
    ...
    for epoch in range(1, 51):
        ...
        if ema_val_ap > best_ema_val_ap:
            ...
            torch.save({'memory': memory.state_dict(), 'gnn': gnn.state_dict(), 'link_pred': link_pred.state_dict()}, osp.join('saved_models', f'bitcoin_tgn_seed_{seed}.pth'))
        else:
...
if __name__ == '__main__':
    ...
    try:
        log_filename = f"bitcoin_log_{time.strftime('%Y%m%d-%H%M%S')}.txt"
        tee = Tee(log_filename)
        
        # --- Custom Data Loading Script ---
        path = osp.join('.', 'data', 'BitcoinOTC') # PyG will download and process the data here
    
        print("Loading BitcoinOTCDataset dataset...")
        # dataset = BitcoinOTCDataset(path)
        dataset = BitcoinOTCDataset(root='./data/')
