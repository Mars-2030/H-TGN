...
def plot_tsne_node_embeddings(history, tracked_nodes, seed):
    ...
    plt.tight_layout()
    plot_filename = f"icews18_tgn_seed_{seed}.png"
...
def run_single_experiment(seed, data, train_data, val_loader, test_loader, device, loader_kwargs):
    ...
    for epoch in range(1, 51):
        ...
        if ema_val_ap > best_ema_val_ap:
            ...
            torch.save({'memory': memory.state_dict(), 'gnn': gnn.state_dict(), 'link_pred': link_pred.state_dict()}, osp.join('saved_models', f'icews18_tgn_seed_{seed}.pth'))
        else:
...
if __name__ == '__main__':
    ...
    try:
        log_filename = f"icews18_log_{time.strftime('%Y%m%d-%H%M%S')}.txt"
        tee = Tee(log_filename)
        
        # --- Custom Data Loading Script ---
        path = osp.join('.', 'data', 'ICEWS18') # PyG will download and process the data here
    
        print("Loading ICEWS18 dataset...")
        # dataset = ICEWS18(path)
