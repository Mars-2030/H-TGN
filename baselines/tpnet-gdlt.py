...
# ----- Link predictor (kept the same interface) -----
...
def plot_tsne_node_embeddings(history, tracked_nodes, seed):
    ...
    plt.tight_layout()
    plot_filename = f"gdelt_tpnet_seed_{seed}.png"
    plt.savefig(plot_filename, dpi=300)
    print(f"t-SNE plot saved to {plot_filename}")


# ============================ TPNet Train / Eval loops ============================
# Keep function names the same; unused args are accepted for compatibility.

def _tpnet_forward_batch(tp_model: TPNet,
                         link_pred: LinkPredictor,
                         src_np: np.ndarray, dst_np: np.ndarray, t_np: np.ndarray,
                         neg_dst_np: Optional[np.ndarray] = None):
    """
    Helper that runs TPNet to get embeddings & link logits for a positive (and optional negative) set.
    """
    # Compute node embeddings at the batch times:
    ...
    if neg_dst_np is None:
        return pos_out, None
    # negatives: pair the same sources with corrupt destinations at same timestamp
...
def train_epoch(loader, memory, gnn, link_pred, optimizer, neighbor_loader, assoc, data_full, criterion):
    """
    Kept the same signature for compatibility with your main() call.
    This trains TPNet instead of TGN/GraphMixer internally.
    """
    tp_model: TPNet = gnn  # reuse the 'gnn' slot to hold our TPNet instance
    tp_model.train(); link_pred.train()

    # IMPORTANT: start from the initial RP state each epoch
    ...
    for batch in loader:
        optimizer.zero_grad()
        # batch fields: src, dst, t, neg_dst (from TemporalDataLoader)
        src_np = batch.src.cpu().numpy()
        dst_np = batch.dst.cpu().numpy()
        t_np   = batch.t.cpu().numpy().astype(np.float64)
        neg_np = batch.neg_dst.cpu().numpy()

        pos_out, neg_out = _tpnet_forward_batch(tp_model, link_pred, src_np, dst_np, t_np, neg_np)
        ...
        # After prediction and update of weights, advance RP state with *this* batch (no leakage):
        if tp_model.random_projections is not None:
...
def evaluate(loader, memory, gnn, link_pred, neighbor_loader, assoc, data_full, criterion, nnodes: int):
    """
    Kept signature; evaluates TPNet.
    Carries RP state forward in chronological order across the evaluation split.
    """
    tp_model: TPNet = gnn
    tp_model.eval(); link_pred.eval()

    # reset RP at eval start
    if tp_model.random_projections is not None:
        tp_model.random_projections.reset_random_projections()
    ...
    for batch in loader:
        ...
        # 2-class rank: compare pos vs neg score for each row
        ...
        # advance RP with this batch
        if tp_model.random_projections is not None:
            tp_model.random_projections.update(src_np, dst_np, t_np)

    # No full node-bank here; return empty tensor and fill in run_single_experiment.
...
def run_single_experiment(seed, data_full, train_data, val_loader, test_loader, device, loader_kwargs, nnodes: int):
    ...
    # Data arrays
    src_np = data_full.src.cpu().numpy().astype(np.int64)
    dst_np = data_full.dst.cpu().numpy().astype(np.int64)
    t_np   = data_full.t.cpu().numpy().astype(np.float64)

    # fabricate unique edge IDs per event (1..M) so sampler can reference edge features
    M = len(src_np)
    edge_ids = np.arange(1, M + 1, dtype=np.int64)

    # Build the sampler using tpnet helper (expects these four arrays on a data-like object)
    sampler_data = NS(
        src_node_ids=src_np,
        dst_node_ids=dst_np,
        edge_ids=edge_ids,
    ...
    # Node & edge raw features (fixed, as TPNet expects numpy arrays):
    # Node features: random normal (Dn) with 0th row as PAD
    Dn = 128
    node_raw_features = np.zeros((nnodes + 1, Dn), dtype=np.float32)
    rng = np.random.default_rng(seed)
    node_raw_features[1:] = rng.normal(0.0, 1.0, size=(nnodes, Dn)).astype(np.float32)

    # Edge features: we map each event (edge_id) to its relation message (shape 1)
    De = int(data_full.msg.size(-1))
    edge_raw_features = np.zeros((M + 1, De), dtype=np.float32)  # 0th row PAD
    edge_raw_features[1:] = data_full.msg.cpu().numpy().astype(np.float32)

    # Random projection module
    rp = RandomProjectionModule(
        node_num=nnodes,            # count of nodes (not max ID)
        edge_num=M,
        dim_factor=8,               # tune
        num_layer=2,                # K-hop depth
        time_decay_weight=0.1,      # lambda
        device=str(device),
        use_matrix=False,           # keep projected representation (fast)
        beginning_time=np.float64(t_np.min()),
        not_scale=False,
        enforce_dim=-1
    ).to(device)
    ...
    # Optimizer/scheduler/criterion (unchanged)
    ...
    for epoch in range(1, 51):
        ...
        train_loss, train_pos_loss, train_neg_loss = train_epoch(
            TemporalDataLoader(train_data, batch_size=200, neg_sampling_ratio=1.0, **loader_kwargs),
            memory, tpnet, link_pred, optimizer, neighbor_loader, assoc, data_full, criterion
        )
        ...
        if ema_val_ap > best_ema_val_ap:
            ...
            torch.save(
                ...
                osp.join('saved_models', f'gdelt_tpnet_seed_{seed}.pth')
            )
        else:
...
log_filename = f"gdelt_tpnet_seed_{time.strftime('%Y%m%d-%H%M%S')}.txt"
tee = Tee(log_filename)
print(f"Using device: {device}")

# ---- Load GDELT ----
dataset_root = osp.join('.', 'data')
dataset = GDELTDataset(dataset_root, bin_15min=True)
data = dataset[0]

# FIX: node count must be max_id + 1 (IDs are 0..max_id)
nnodes = int(max(int(data.src.max()), int(data.dst.max()))) + 1
print(f"#Nodes inferred: {nnodes} | #Events: {data.t.numel()}")
print(f"ID stats → src: [{int(data.src.min())},{int(data.src.max())}], "
...
tracked_nodes = np.random.choice(nnodes + 1, num_nodes_to_track, replace=False)  # include 0 maybe
...
for seed in seeds:
    ...
    # We didn't capture full node banks in eval; skip t-SNE unless you add a capture.
    # If you want t-SNE, you can call TPNet to encode a fixed tracked set at a chosen time here.
