# ---- Non-Kaggle Bitcoin-OTC dataset class (clean) ----
...
class BitcoinOTCDataset(InMemoryDataset):
    ...
    def __repr__(self) -> str:
        return 'BitcoinOTC()'
# ========================== TPNet on Bitcoin-OTC: Training Module ==========================


# ---- import dataset + tpnet ----
# from bitcoin_otc_dataset import BitcoinOTCDataset   # <-- adjust module name if needed
...
def train_epoch(loader, memory, gnn, link_pred, optimizer, neighbor_loader, assoc, data_full, criterion,
                nnodes: int, id_map_arr: np.ndarray):
    """Train TPNet. id_map_arr maps raw IDs -> remapped 1..N IDs (0 reserved as PAD)."""
    tp_model: TPNet = gnn
    ...
    for batch in loader:
        ...
        # Map raw -> remapped
        src_np_raw = batch.src.cpu().numpy()
        dst_np_raw = batch.dst.cpu().numpy()
        neg_np_raw = batch.neg_dst.cpu().numpy()
        t_np       = batch.t.cpu().numpy().astype(np.float64)

        src_np = id_map_arr[src_np_raw]
        dst_np = id_map_arr[dst_np_raw]
        neg_np = id_map_arr[neg_np_raw]
...
def evaluate(loader, memory, gnn, link_pred, neighbor_loader, assoc, data_full, criterion,
    ...
    for batch in loader:
        src_np = id_map_arr[batch.src.cpu().numpy()]
        dst_np = id_map_arr[batch.dst.cpu().numpy()]
        neg_np = id_map_arr[batch.neg_dst.cpu().numpy()]
        t_np   = batch.t.cpu().numpy().astype(np.float64)
...
# ----- One run (per seed) -----
def run_single_experiment(seed, data_full, train_data, val_loader, test_loader, device, loader_kwargs):
    print(f"\n{'='*30} RUNNING SEED {seed} {'='*30}")
    set_seed(seed)
    if device.type == 'cuda':
        torch.cuda.reset_peak_memory_stats(device)

    # ------- Build a stable raw->contiguous ID map (reserve 0 as PAD) -------
    raw_src = data_full.src.cpu().numpy().astype(np.int64)
    raw_dst = data_full.dst.cpu().numpy().astype(np.int64)
    uniq = np.unique(np.concatenate([raw_src, raw_dst]))
    # map raw ids to 1..N (0 is PAD)
    N = uniq.shape[0]
    raw_max = int(max(raw_src.max(), raw_dst.max()))
    id_map_arr = np.zeros(raw_max + 1, dtype=np.int64)  # assumes ids are non-negative ints
    id_map = {rid: i+1 for i, rid in enumerate(uniq)}   # 1..N
    for k, v in id_map.items():
        id_map_arr[k] = v

    print(f"[seed {seed}] raw id min={uniq.min()}, max={uniq.max()}, unique={N}")

    # Remapped arrays for sampler
    src_all = id_map_arr[raw_src]
    dst_all = id_map_arr[raw_dst]
    ...
    rp = RandomProjectionModule(
        node_num=nnodes,          # count incl. PAD row 0
        edge_num=M,
        dim_factor=8,
        num_layer=2,
    ...
    # Dummy placeholders to satisfy previous signatures:
    memory = None
    ...
    for epoch in range(1, 51):
        ...
        train_loader = TemporalDataLoader(
            train_data, batch_size=200, neg_sampling_ratio=1.0,
            **loader_kwargs
        )
        ...
        val_loader = TemporalDataLoader(
            val_data, batch_size=200, neg_sampling_ratio=1.0,
            **loader_kwargs
        )
        val_loss, val_ap, val_auc, val_mrr, _ = evaluate(
            val_loader, memory, tpnet, link_pred, neighbor_loader, assoc, data_full, criterion,
            nnodes, id_map_arr
        )
        ...
        # Evaluate on test when we get a new best EMA on val
        if ema_val_ap > best_ema_val_ap:
            best_ema_val_ap = ema_val_ap
            patience_counter = 0
            test_loader = TemporalDataLoader(
                test_data, batch_size=200, neg_sampling_ratio=1.0,
                **loader_kwargs
            )
            _, test_ap, test_auc, test_mrr, _ = evaluate(
                test_loader, memory, tpnet, link_pred, neighbor_loader, assoc, data_full, criterion,
                nnodes, id_map_arr
            )
            ...
            torch.save(
                ...
                osp.join('saved_models', f'bitcoinotc_tpnet_seed_{seed}.pth')
            )
        else:
...
log_filename = f"bitcoinotc_tpnet_seed_{time.strftime('%Y%m%d-%H%M%S')}.txt"
tee = Tee(log_filename)
print(f"Using device: {device}")

# ---- Load dataset ----
dataset_root = osp.join('.', 'data')
dataset = BitcoinOTCDataset(dataset_root)
data = dataset[0]

# Info + cleanup
raw_nnodes_guess = int(max(int(data.src.max()), int(data.dst.max()))) + 1
print(f"#Events: {data.t.numel()} | Raw max-id-based nnodes guess: {raw_nnodes_guess}")
print(f"Raw ID stats → src: [{int(data.src.min())},{int(data.src.max())}], "
      f"dst: [{int(data.dst.min())},{int(data.dst.max())}]")

# Remove any persisted scalars
strip_scalar_attrs(data)

# Time-respecting split
train_data, val_data, test_data = data.train_val_test_split(val_ratio=0.15, test_ratio=0.15)

# DataLoader settings (avoid worker teardown warnings in notebooks)
loader_kwargs = dict(
...
all_results = []
for seed in seeds:
    result, _ = run_single_experiment(seed, data, train_data, val_loader=None, test_loader=None,
                                      device=device, loader_kwargs=loader_kwargs)
