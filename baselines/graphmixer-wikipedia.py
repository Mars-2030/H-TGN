# ========= ICEWS18 + GraphMixer training module (JODIE-configurable) =========
import os
...
# NEW: needed for GraphMixerEncoder
from torch_scatter import scatter_mean
from torch.nn import Linear as _Linear, LayerNorm, GELU, Dropout
import argparse
...
class GraphMixerEncoder(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, msg_dim, time_enc,
        ...
        # === FeatEncode ===
        self.feat_linear = _Linear(token_in, hidden)

        # === MixerBlock (token then channel) ===
        # token FFN: hidden -> 0.5*hidden -> hidden
        self.token_ln   = LayerNorm(hidden)
        self.token_ffn0 = _Linear(hidden, int(0.5 * hidden))
        self.token_ffn1 = _Linear(int(0.5 * hidden), hidden)

        # channel FFN: out_dim -> 4*out_dim -> out_dim
        self.channel_ln   = LayerNorm(hidden)
        self.channel_ffn0 = _Linear(hidden, 4 * hidden)
        self.channel_ffn1 = _Linear(4 * hidden, hidden)

        # project to output dim and add residual from memory
        self.proj_out = _Linear(hidden, out_channels)
        self.res_proj = _Linear(in_channels, out_channels)
...
# (Kept for structure consistency, but unused now)
...
def plot_tsne_node_embeddings(history, tracked_nodes, seed, dataset_name: str):
    if not history:
    ...
    plt.tight_layout()
    plot_filename = f"{dataset_name}_tgn_seed_{seed}.png"
...
def run_single_experiment(seed, data, train_data, val_loader, test_loader, device, loader_kwargs, dataset_name: str):
    ...
    memory = TGNMemory(
        data.num_nodes, data.msg.size(-1), memory_dim, time_dim,
        message_module=IdentityMessage(data.msg.size(-1), memory_dim, time_dim),
        aggregator_module=LastAggregator()
    ).to(device)

    # SWITCHED to GraphMixerEncoder
    ...
    for epoch in range(1, 51):
        ...
        if ema_val_ap > best_ema_val_ap:
            ...
            torch.save(
                ...
                osp.join('saved_models', f'{dataset_name}_tgn_seed_{seed}.pth')
            )
        else:
...
if __name__ == '__main__':
    DATASET_NAME = 'wikipedia'   # <-- change this to the dataset you want
    dataset_name = os.environ.get('JODIE_NAME', DATASET_NAME)
    ...
    try:
        log_filename = f"{dataset_name}_tgn_seed_{time.strftime('%Y%m%d-%H%M%S')}.txt"
        ...
        all_results = []
        for seed in seeds:
            result, history = run_single_experiment(seed, data, train_data, val_loader, test_loader, device, loader_kwargs, dataset_name)
            ...
            plot_tsne_node_embeddings(tracked_history, tracked_nodes, seed=seed, dataset_name=dataset_name)
