...
tqdm = lambda *args, **kwargs: args[0]  # disables progress bars
...
sys.argv = ['notebook', '--dataset_name', 'wikipedia', '--model_name', 'DyGFormer', '--num_epochs', '51', '--patience', '15', '--dropout', '0.1']
...
# Training sampler: only train edges
...
# Validation sampler: only edges up to validation split
val_neighbor_sampler = get_neighbor_sampler(
    data=full_data,  # includes everything
    sample_neighbor_strategy=args.sample_neighbor_strategy,
    time_scaling_factor=args.time_scaling_factor,
    seed=1
)

test_neighbor_sampler = get_neighbor_sampler(
    data=full_data,
    sample_neighbor_strategy=args.sample_neighbor_strategy,
    time_scaling_factor=args.time_scaling_factor,
    seed=2
...
# Training negatives: only train nodes
train_neg_edge_sampler = NegativeEdgeSampler(
    src_node_ids=train_data.src_node_ids,
    dst_node_ids=train_data.dst_node_ids
)

# Validation negatives: only nodes seen up to validation
val_neg_edge_sampler = NegativeEdgeSampler(
    src_node_ids=np.concatenate([train_data.src_node_ids, val_data.src_node_ids]),
    dst_node_ids=np.concatenate([train_data.dst_node_ids, val_data.dst_node_ids]),
    seed=0
)

# Inductive validation negatives: only nodes in new-node validation set
...
# Test negatives: only nodes seen up to test
test_neg_edge_sampler = NegativeEdgeSampler(
    src_node_ids=np.concatenate([train_data.src_node_ids, val_data.src_node_ids, test_data.src_node_ids]),
    dst_node_ids=np.concatenate([train_data.dst_node_ids, val_data.dst_node_ids, test_data.dst_node_ids]),
    seed=2
)

# Inductive test negatives: only nodes in new-node test set
...
for run in range(args.num_runs):
 ...
 #               f'{get_parameter_sizes(model) * 4 / 1024} KB, {get_parameter_sizes(model) * 4 / 1024 / 1024} MB.')
        ...
        for batch_idx, train_data_indices in enumerate(train_idx_data_loader):
            ...
            train_metrics.append(
                get_link_prediction_metrics(
                    predicts=predicts,
                    labels=labels,
                    query_ids = np.concatenate([batch_src_node_ids, batch_neg_src_node_ids])
    ...
    peak_memory = torch.cuda.max_memory_allocated() / (1024 ** 2) if torch.cuda.is_available() else 0
        ...
        # extract test AP, AUC, MRR
    ap = test_metric_dict.get("average_precision", np.nan)
    auc = test_metric_dict.get("roc_auc", np.nan)
    mrr = test_metric_dict.get("mrr", np.nan)  # will be NaN unless you add MRR calculation

    # store row for summary table
    summary_rows.append([ap, auc, mrr, avg_epoch_time, peak_memory])

    print(f"\nRun {run+1} Results:")
    print(f"AP={ap:.4f}  AUC={auc:.4f}  MRR={mrr:.4f}  "
          f"Avg. Runtime/Epoch={avg_epoch_time:.2f}s  Peak Mem={peak_memory:.2f}MB\n")

aps, aucs, mrrs, runtimes, mems = map(np.array, zip(*summary_rows))

print("\n===== Final Summary over Runs =====")
print(f"{'Metric':<20}{'Mean':<15}{'Std':<15}")
print(f"{'AP':<20}{aps.mean():<15.4f}{aps.std(ddof=1):<15.4f}")
print(f"{'AUC':<20}{aucs.mean():<15.4f}{aucs.std(ddof=1):<15.4f}")
print(f"{'MRR':<20}{mrrs.mean():<15.4f}{mrrs.std(ddof=1):<15.4f}")
print(f"{'Avg. Runtime/Epoch (s)':<20}{runtimes.mean():<15.2f}{runtimes.std(ddof=1):<15.2f}")
print(f"{'Peak Memory (MB)':<20}{mems.mean():<15.2f}{mems.std(ddof=1):<15.2f}")
